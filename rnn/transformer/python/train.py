...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default='./models/transformer.pt', type=str, help="The output directory where the model checkpoints will be written.")
    ...
    print('lang1_vocab_vocab_size', dataset.lang1_vocab_size)
    print('lang2_vocab_size', dataset.lang2_vocab_size)
    ...
    def collate_fn(batch):
        ...
        source = pad_sequence(source, batch_first=True, padding_value=dataset.lang1_vocab['<pad>']) 
        
        #get all target indexed sentences of the batch
        target = [item[1] for item in batch] 
        #pad them using pad_sequence method from pytorch. 
        target = pad_sequence(target, batch_first=True, padding_value=dataset.lang2_vocab['<pad>'])
    ...
    model = Transformer(d_model, n_layers, dataset.lang1_vocab_size, dataset.lang2_vocab_size, d_k, d_v, n_heads, d_ff, device).to(device)
    criterion = nn.CrossEntropyLoss(ignore_index=dataset.lang2_vocab['<pad>'])
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train(model, train_loader, optimizer, criterion, dataset.lang2_vocab)
        valid_loss = evaluate(model, val_loader, criterion, dataset.lang2_vocab)
